...
int main(int argc, char** argv)
{
	...
	setlocale(LC_ALL, "Russian");// ”становка русской локали

	cout << zagolovok << endl;
	cout << "[+] =====================================================" << endl;
	cout << "[+] KEYHUNT-CUDA v"<<RELEASE<<"                         "<<DATA_RELIZA << endl;
	cout << "[+] =====================================================\n" << endl;
	cout << grey;
	...
	//если название файла outputFile(Found.txt) не изменЄно добавим в название начало(5 знаков) и конец(5 знаков) диапазона вида Found_80001_80002.txt
	if (outputFile == "Found.txt") {
		std::string rangeStartHex = rangeStart.GetBase16();
		std::string rangeENDHex = rangeEnd.GetBase16();

		if (rangeStartHex.length() > 5){ //≈сли начальное вообще больше 5 
			rangeStartHex  = rangeStartHex.substr(0, 5);
		}
		if (rangeENDHex.length() > 5) { //≈сли конечное вообще больше 4 хз проверим )) 
