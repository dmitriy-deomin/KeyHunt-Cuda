	...
	const std::string& rangeStart, const std::string& rangeEnd, bool& should_exit)
{
	...
	while (i < N && !should_exit) {
		...
		if (fread(buf, 1, K_LENGTH, wfd) == K_LENGTH) {
			...
			memcpy(DATA + (nr * K_LENGTH), buf, K_LENGTH); // Используем nr вместо i

			nr++; // Инкрементируем после успешного чтения
	...
	TOTAL_COUNT = N;
	targetCounter = i;
	printf("\n"); // Перевод строки после завершения работы
...
void KeyHunt::Search(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize, bool& should_exit)
{
	...
	while (isAlive(params)) {
		...
		if (rKey == 0) {
		  ...
		  /*
		   Чтобы подсчитать сколько осталось секунд для конца перебора :
